...
│   ├── opencode.nix            # Opencode CLI configuration
...
│   └── autostart.nix           # XDG autostart entries (teams-for-linux)
├── cachix/
...
- **Automatic garbage collection enabled** - runs weekly via `nh.clean`, keeps generations from the last 30 days (`--keep-since 30d`)
- Enabled 2026-09-21 after `/boot` (1GB EFI System Partition) filled up completely with old kernel/initrd pairs from ~20 accumulated generations, breaking `nhs`/`nixos-rebuild boot`
...
- **users.nix**: User accounts (joemitz with groups: networkmanager, wheel, docker, kvm, input; root), timezone (America/Los_Angeles), locale, polkit, passwordless sudo. Input group membership enables dotool keystroke injection for Handy dictation on Wayland
- **secrets.nix**: Complete sops-nix configuration for encrypted secrets management
- **services.nix**: Docker, NFS client, nix-ld (for Android SDK tools), 1Password CLI and GUI (with polkit), Bluetooth autopower service (powers on adapter after boot to workaround CSR dongle timeout), uinput kernel module and udev rules for dotool (enables keystroke injection for Handy dictation on Wayland/KWin), NH (Nix Helper), Nix settings (experimental features, trusted-users for signing and remote builds)
...
- nix-ld enabled for running Android SDK dynamically linked executables (AAPT2, CMake, Ninja, NDK tools) with graphics support (libgbm)
