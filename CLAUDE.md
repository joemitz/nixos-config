...
└── pkgs/
    ...
    └── handy.nix              # Custom Handy AppImage wrapper
```

**Flake Structure**:
- `flake.nix`: Main entry point defining inputs (nixpkgs stable, home-manager, claude-code, sops-nix, tiny4linux, impermanence, pinned nixpkgs for tiny4linux and handy) and outputs
...
- `pkgs/handy.nix`: Custom package for Handy AppImage wrapper
...
- Pinned nixpkgs inputs for heavy dependencies to avoid unnecessary rebuilds:
  ...
  - `nixpkgs-handy`: Pinned to 063f43f (avoid flake update rebuilds)
...
After making configuration changes, Claude Code should run a dry-run build to verify the configuration compiles:
```bash
nixos-rebuild dry-build --flake .#nixos
```
This command:
- Tests that the configuration builds successfully without applying it
- Catches syntax errors, missing packages, or broken references
- Provides early feedback before the user runs `nhs` or `nhb`
- Does not require sudo or make any system changes

Only after the dry-run build passes should Claude Code tell the user to run `nhs` or `nhb`.
...
- **packages.nix**: All user packages - CLI tools (claude-code, gh, jq, awscli2, awslogs, devbox, nodejs_24, btop, eza, wtype), Nix tools (nixd, nixpkgs-fmt, nixf, statix, deadnix, sops), development apps (vscodium, postman, android-studio, android-tools, jdk17), applications (zoom-us, tidal-hifi, vlc, gimp, guvcview, remmina), custom packages (tiny4linux, handy). Uses pinned nixpkgs inputs for custom packages to avoid unnecessary rebuilds (tiny4linux from nixpkgs-tiny4linux to avoid Rust updates, handy from nixpkgs-handy to avoid flake update rebuilds). Module header cleaned to include only required parameters. Note: tmux enabled via programs.tmux in tmux.nix, not listed here
...
- TrueNAS Kopia share mounted at /mnt/truenas/kopia (Kopia backups, read-write, automounted on access)
...
- Wayland backend configured via extraBwrapArgs (GDK_BACKEND set to wayland, Wayland sockets bound for display authorization)
- Single-arch binary (multiArch disabled for simpler Wayland-only operation)
- Desktop entry created for easy access
