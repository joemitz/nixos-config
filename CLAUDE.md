...
- **packages.nix**: All user packages - CLI tools (claude-code, gh, jq, patchelf, devbox, nodejs_24, micro, btop, eza), development apps (vscodium, postman, android-studio, android-tools, jdk11), applications (zoom-us, tidal-hifi, vlc, gimp, guvcview, remmina), custom packages (tiny4linux). Note: tmux enabled via programs.tmux in tmux.nix, not listed here
